...
import {
  OrganizationType,
  Industry,
} from '../../common/constants/organization-types';
...
export class CreateOrganizationDto {
  ...
  @Matches(/^[a-zA-Z0-9\s\-\&\.\,\(\)]+$/, {
    message:
      'Organization name can only contain letters, numbers, spaces, hyphens, ampersands, periods, commas, and parentheses',
  })
  name: string;
  ...
  @IsEnum(OrganizationType, {
    message:
      'Organization type must be one of: semicon, corporate, startup, university, government, other',
  })
  type: OrganizationType;

  @IsEnum(Industry, {
    message:
      'Industry must be one of: IT, Telecom, Healthcare, Finance, Education, Manufacturing, Retail, Automotive, Energy, Agriculture, Other',
  })
  industry: Industry;
  ...
  @Matches(/^[a-zA-Z\s\-'\.]+$/, {
    message:
      'POC name can only contain letters, spaces, hyphens, apostrophes, and periods',
  })
  poc_name: string;
  ...
  @IsEmail(
    {},
    { message: 'POC email must be in valid format (e.g., contact@domain.com)' },
  )
  poc_email: string;
