import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Request, NotFoundException, ForbiddenException, HttpStatus, HttpCode, BadRequestException, ConflictException, Query, Res } from '@nestjs/common';
...
export class OrganizationsController {
  ...
  async findAll(
    ...
    const result = await this.organizationsService.findOrganizationsWithPagination(queryDto, req.user);
  ...
  async findOne(@Param('id') id: string, @Request() req) {
    ...
    // ClientAdmin can only see their own organization
      ...
      throw new ForbiddenException('Not authorized to access this organization');
    }
  ...
  @Get(':id/users')
  @Roles(UserRole.PLATFORM_ADMIN, UserRole.CLIENT_ADMIN)
  async getOrganizationUsers(@Param('id') id: string, @Request() req, @Res() res: Response) {
    const orgId = +id;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to access users from this organization');
    }
  ...
  @Roles(UserRole.PLATFORM_ADMIN, UserRole.CLIENT_ADMIN)
  @HttpCode(HttpStatus.CREATED)
  async createOrganizationUser(@Param('id') id: string, @Body() createUserDto: CreateUserDto, @Request() req) {
    const orgId = +id;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to create users in this organization');
    }
  ...
  getOrganizationUser(@Param('id') id: string, @Param('userId') userId: string, @Request() req) {
    const orgId = +id;
    ...
    if ((req.user.role === UserRole.MANAGER || req.user.role === UserRole.LEARNER) && userIdNum !== req.user.userId) {
  ...
  @HttpCode(HttpStatus.OK)
  async updateOrganizationUser(@Param('id') id: string, @Param('userId') userId: string, @Body() updateUserDto: UpdateUserDto, @Request() req) {
    const orgId = +id;
    const userIdNum = +userId;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to update users in this organization');
    }
    ...
    await this.usersService.updateInOrganization(userIdNum, orgId, updateUserDto);
  ...
  removeOrganizationUser(@Param('id') id: string, @Param('userId') userId: string, @Request() req) {
    const orgId = +id;
    const userIdNum = +userId;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to delete users from this organization');
    }
  ...
  @Roles(UserRole.PLATFORM_ADMIN, UserRole.CLIENT_ADMIN)
  async searchOrganizationUsers(@Param('id') id: string, @Param('query') query: string, @Request() req, @Res() res: Response) {
    const orgId = +id;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to search users in this organization');
    }
  ...
  @Roles(UserRole.PLATFORM_ADMIN, UserRole.CLIENT_ADMIN)
  async filterOrganizationUsersByRole(@Param('id') id: string, @Param('role') role: string, @Request() req, @Res() res: Response) {
    const orgId = +id;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to filter users in this organization');
    }
  ...
  getOrganizationUserStats(@Param('id') id: string, @Request() req) {
    ...
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to view stats for this organization');
    }
  ...
  async getManagerTeamInOrganization(@Param('id') id: string, @Param('managerId') managerId: string, @Request() req, @Res() res: Response) {
    const orgId = +id;
  ...
  async getOrganizationUserCount(@Param('id') id: string, @Request() req) {
    ...
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to view user count for this organization');
    }
  ...
  @Roles(UserRole.PLATFORM_ADMIN, UserRole.CLIENT_ADMIN)
  async getOrganizationUserCountByRole(@Param('id') id: string, @Request() req) {
    const orgId = +id;
    
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to view user statistics for this organization');
    }
    
    const userCountByRole = await this.organizationsService.getUserCountByRole(orgId);
    return {
      org_id: orgId,
  ...
  async getOrganizationStatistics(@Param('id') id: string, @Request() req) {
    ...
    // Check if user has access to this organization
    if (req.user.role === UserRole.CLIENT_ADMIN && req.user.orgId !== orgId) {
      throw new ForbiddenException('Not authorized to view statistics for this organization');
    }
    
    const statistics = await this.organizationsService.getOrganizationStatistics(orgId);
