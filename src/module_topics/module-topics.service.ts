...
export class ModuleTopicsService {
  ...
  async unlinkTopic(moduleId: number, topicId: number) {
    ...
    const res = await this.mtRepo.delete({ module_id: moduleId, topic_id: topicId });
    if (res.affected === 0) {
      throw new NotFoundException(`Topic ${topicId} not linked to module ${moduleId}`);
    }
    return { message: 'Unlinked', moduleId, topicId };
  }
  ...
  async unlinkTopics(moduleId: number, topicIds: number[]) {
    ...
    return {
      message: `Successfully unlinked ${result.affected} topic(s) from module ${moduleId}`,
